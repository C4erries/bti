...
- Клиентский заказ:
  ...
  - `calculatorInput` в обоих сценариях (`/calc/estimate` и предварительная оценка заказа)
    использует единый формат: `area`, `works` (булевы `walls`, `wet_zone`, `doorways`),
    `features` (булевы `basement`, `join_apartments`), `urgent`, `notes`;
...
- Справочники:
  ...
  - демо-округа: Центральный, Западный, Прикубанский, Карасунский (коды: central, west, prikub, karasun);
  - демо-типы домов: Панельный дом (panel), Кирпичный дом (brick).
  - при обновлении демо-значений в существующей БД нужно пересоздать записи в таблицах `districts` и `house_types`
    (или повторно вызвать `init_data.py`, который через upsert обновит названия по коду).
