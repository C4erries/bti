...
Актуальное описание предметной области смотри в `domain.md`. При изменении домена сначала обновляй `domain.md`, затем код и OpenAPI.
...
- Поддерживать и развивать REST‑API:
  ...
  - Публичные справочники: `/api/v1/districts`, `/api/v1/house-types`.
  ...
  - Админка:
    ...
    - `/api/v1/admin/orders`,
    - `/api/v1/admin/districts`, `/api/v1/admin/house-types`.
- Поддерживать доменную модель (см. `domain.md`):
  - `User`, `ClientProfile`, `ExecutorProfile`,
  - `Department`, `District`, `HouseType`,
...
- Калькулятор стоимости:
  ...
  - калькулятор не использует отдельную сущность «услуга»:
    стоимость определяется на основе `districtCode`, `houseTypeCode` и `calculatorInput`
    (площадь, виды работ, особенности, срочность и т.п.).
- AI/визуализация:
...
- Вводить новые сущности/поля в доменной модели без фиксации в `domain.md` и `problem.md`.
...
- Развивать UI поверх существующего API:
  ...
  - Для клиента:
    - мастер создания заказа (выбор округа, типа дома, параметров работ),
  ...
  - Для администратора:
    ...
    - модерация заказов;
    - управление справочниками (округа, типы домов);
...
- При создании заказов:
  ...
  - предоставлять выбор округов, типов домов через списки, а не ввод кодов;
...
- Важно:
  - фиксировать ключевые допущения в комментариях или markdown‑файлах (например, в `problem.md` или `domain.md`);
